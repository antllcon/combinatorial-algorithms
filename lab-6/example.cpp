...
// Вычисляет евклидово расстояние между двумя городами по их координатам
...
// Проверяет, существует ли ребро между двумя городами
bool ACO::exists(int cityi, int cityc) { return (GRAPH[cityi][cityc] == 1); }

// Проверяет, посещал ли муравей antk город c
...
// Вычисляет вероятность перехода муравья antk из cityi в cityj
// учитывает феромон и эвристику (1/расстояние), нормируя по всем доступным городам
...
// Вычисляет общую длину маршрута муравья antk
...
// Выбирает следующий город на основе распределения вероятностей PROBS
int ACO::city()
{
...
// Формирует маршрут для муравья antk, начиная с INITIALCITY
...
// Проверка корректности сгенерированного маршрута
// возвращает код ошибки или 0, если маршрут валиден
...
// Печать результатов оптимизации: лучший найденный маршрут
...
// Обновление феромонов по всем муравьям: испарение + отложение нового феромона
...
// Основной цикл оптимизации: запускаем муравьёв, обновляем феромоны
